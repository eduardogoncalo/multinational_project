...
def get_input_data()->pd.DataFrame:
    df1 = pd.read_csv("data/raw/AppGallery.csv", skipinitialspace=True)
    df1.rename(columns={'Type 1': 'y1', 'Type 2': 'y2', 'Type 3': 'y3', 'Type 4': 'y4'}, inplace=True)
    df2 = pd.read_csv("data/raw/Purchasing.csv", skipinitialspace=True)
...
def save_data(df, save_path)-> pd.DataFrame:
...
def translate_to_en(texts: list) -> list:
    """
    Função wrapper que utiliza a nossa classe modular para traduzir textos.
    """
    print("Iniciando tradução usando a classe EnglishTranslator...")
    
    # 1. Instanciamos a classe que já consertamos (sem o bug do pipeline e sem o bug do NaN)
    translator = EnglishTranslator(use_case=2)
    
    # 2. Usamos o método transform para fazer o trabalho pesado
    translated_texts = translator.transform(texts)
